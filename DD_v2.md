# PWeb — Personal Portfolio Site: Design Doc v2 (Simplified, Sparse-Card + Detail-Page Architecture)

## What changed from v1, and why (context for whoever reads this later)
...
### 4.1 Hero section (unchanged in spirit from v1)
- Casual tone, not LinkedIn-header style.
- Photo (placeholder this pass) + short 3-line description (filler text this pass) + link row (GitHub, LinkedIn, résumé, email — `#` placeholders).
- Sits on the constellation background (§5), establishing the site's visual identity immediately.
- No filter bar below it anymore (removed per §2.3) — the hero transitions directly into the project grid.
...
### 4.4 Layout
- Grid or stacked-large-card layout, similar in spirit to the reference site's project section — large imagery, generous spacing, not dense. Scroll-triggered fade/slide-in as cards enter viewport (`IntersectionObserver`-based, standard pattern, no custom Svelte state needed for this — can be done with a small vanilla JS snippet or a lightweight Astro/Svelte component).

---

## 5. Visual Identity (site-wide, consistent — replaces v1's per-block theme system)

### 5.1 Background: constellation starfield
- Real constellations (actual star position data — Orion, Cassiopeia, Big Dipper, etc.), dimmed/low-opacity, persistent across the whole site (hero, index cards, detail pages) via the shared layout component (§2.2).
- Colors/densities vary across constellations for visual interest.
- This is now the site's **one signature visual element**, carrying the "boldness in one place" role that per-block theming was diluting across many places in v1.

### 5.2 Cursor: "Ship of the Imagination"
- Default cursor treatment sitewide, everywhere, no per-block overrides in this pass (overrides deferred along with the easter eggs — see §8).
...
### 5.3 Color palette
- Since per-block theming is dropped, the site needs **one cohesive palette** rather than four. Not fully specified in this doc — treat as an open item for the next conversation pass (a dark, night-sky-consistent base with one or two accent colors is the obvious direction given the constellation motif, but exact hex values should be proposed and reviewed, not invented silently by Claude Code). Flag chosen values clearly in a theme config file for easy adjustment.
...
- **Site palette**: needs actual hex values decided (§5.3).
...
- [ ] Constellation background renders consistently across all pages.
